function [BPimg_nii,Timg_nii] = dolpntPET_voxelwise(frame_time,frame_dur,ont,offt,TACs,CurPET)
%lp-ntPET analysis
...
%
%   Outputs:
%   BPimg_nii - Dynamic binding potential nifti object
%   Timg_nii - T-statistics image of tracer displacement nifti object
...
Options.gamma_alpha = [.25,0.5,1,10,4]; % "sharpness" or steepness of decay
...
%% Create outputs
BPimg = zeros(temp(1:4));  % 4 D output file
Timg = zeros(temp(1:3));
...
for voxidx=maskidx'
    ...
    if BP_srtm2/se_BP_srtm2>1   % checks that BP is different from 0
        ...
        best_se = se_all(:,opti);
        Fit_lpntpet = [ASRTM2, AAct(:,:,opti)]*best_parest;
        F_SRTM2 = ((mse_srtm2-min_mse)./(ntasks+2-2))./(min_mse./(t_points-(ntasks+2))); % f value relative to srtm2
        
        [x y z] = ind2sub(temp(1:3),voxidx);
        ...
        for idx = 1:ntasks
            ...
            T_fit = T_fit+G/best_se(par_shift+idx);
        end
        T_fit = T_fit/ntasks;
